...
using RakNet.Packets;
using RakNet.Packets.Types;
...
public sealed class NetworkServer : IDisposable
{
    ...
    private readonly ushort _minMtu;
    private readonly ushort _maxMtu;

    private Socket? _socket;
    private CancellationTokenSource? _runCts;

    /// <summary>
    /// Creates a server with <see cref="RaknetServerOptions.Default"/> when
    /// <paramref name="options"/> is null.
    /// </summary>
    public NetworkServer(RaknetServerOptions? options = null)
    {
        Options = options ?? RaknetServerOptions.Default;
        _minMtu = Options.ResolvedMinMtu;
        _maxMtu = Options.ResolvedMaxMtu;
        ServerGuid = Options.ServerGuid != 0
            ? Options.ServerGuid
            : unchecked((ulong)Random.Shared.NextInt64(1, long.MaxValue));
        _frameBufferSize = Math.Max(DefaultFrameBufferSize, (int)_maxMtu);
    }
    ...
    public async ValueTask Start(CancellationToken cancellationToken = default)
    {
        ...
        if (!IPAddress.TryParse(Options.Address, out IPAddress? bindAddress))
        {
        ...
        _socket.Bind(new IPEndPoint(bindAddress, Options.PortIpv4));
        ...
        try
        {
            ...
            {
                SocketAddress endpoint = new(bindAddress.AddressFamily);
    ...
    private void HandleUnconnectedPing(SocketAddress endpoint, ReadOnlySpan<byte> message)
    {
        ...
        string advertisement = Options.BuildAdvertisement(ServerGuid);
    ...
    private void HandleOpenConnectionRequestOne(SocketAddress endpoint, ReadOnlySpan<byte> message)
    {
        ...
        ushort mtu = (ushort)Math.Clamp(message.Length + RakNetHeaderSize, _minMtu, _maxMtu);
    ...
    {
        bool canAccept = Options.CanAcceptConnections?.Invoke() ?? true;
        if (_socket is null || !canAccept || _connections.Count >= Options.MaxConnections || message.Length < 1 + Magic.MAGIC_LENGTH)
        {
            return;
        }
        ...
        ushort selectedMtu = Math.Clamp(request.MTU, _minMtu, _maxMtu);
